...
export class ManagerLeavesComponent implements OnInit {
  ...
  IsAdminandManager() {
    if (this.roleid === 0 || this.roleid === 9) {
      return true;
    } else {
      return false;
    }
  ...
  IsEmployee() {
    if (this.roleid === 2 || this.roleid === 9) {
      return true;
    } else {
      return false;
    }
  ...
  saveAttatchment() {
    if (
      ...
      Swal.fire("Stop", "Please fill the Attachment Name", "error");
      return;
    }
    if (this.fileToUpload == null) {
      Swal.fire("Stop", "Please select the Attachment File", "error");
      return;
    }
